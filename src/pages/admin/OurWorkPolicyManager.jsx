...
export default function OurWorkPolicyManager() {
  ...
  useEffect(() => {
    if (responseData?.data) {
      ...
      setForm({
        title: d.title || "What we want changed, and what we don't.",
        subtitle: d.subtitle || "Policy & Advocacy",
        description: d.description || "Our positions follow directly from the burden of harm. They are the things we believe the UK needs to do, and the framings we believe should be retired from public conversation. We say them plainly.",
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axiosSecure.put("/about/work-policy-banner", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      toast.success("Policy & Advocacy Banner updated successfully!");
      setImageFile(null);
      refetch();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#0F4A63] to-[#156E94] rounded-2xl px-7 py-5 text-white shadow-sm">
        <h1 className="text-xl font-bold tracking-tight">Policy & Advocacy Page CMS</h1>
        <p className="text-white/70 text-sm mt-1">
          Manage the hero banner details, tagline, description, and cover image for the Policy & Advocacy page.
        </p>
      </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="What we want changed, and what we don't."
              className="px-4 py-2.5 rounded-xl border border-slate-200 focus:border-[#156E94] outline-none text-sm transition-all duration-200 text-slate-700 bg-white"
            />
          </div>
