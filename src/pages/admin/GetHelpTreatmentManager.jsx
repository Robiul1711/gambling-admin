...
export default function GetHelpTreatmentManager() {
  ...
  useEffect(() => {
    if (responseData?.data) {
      ...
      setForm({
        title: d.title || "Treatment near you, and online.",
        subtitle: d.subtitle || "Find treatment",
        description: d.description || "UK gambling-harm services have expanded substantially in recent years. They are still unevenly distributed by region. This page sets out what is available where, with national fallbacks for anywhere services are not yet local.",
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axiosSecure.put("/about/get-help-treatment", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      toast.success("Get Help Treatment Banner updated successfully!");
      setImageFile(null);
      refetch();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#0F4A63] to-[#156E94] rounded-2xl px-7 py-5 text-white shadow-sm">
        <h1 className="text-xl font-bold tracking-tight">Find Treatment Page CMS</h1>
        <p className="text-white/70 text-sm mt-1">
          Manage the hero banner details, tagline, description, and cover image for the Find Treatment page.
        </p>
      </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="Treatment near you, and online."
              className="px-4 py-2.5 rounded-xl border border-slate-200 focus:border-[#156E94] outline-none text-sm transition-all duration-200 text-slate-700 bg-white"
            />
          </div>
