...
export default function GetHelpVictimsManager() {
  const axiosSecure = useAxiosSecure();
  const imageInputRef = useRef(null);

  const [form, setForm] = useState({
    title: "", // Bold title prefix
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formData.append("audioTitle", form.audioTitle); // Storing paragraph 2
      if (imageFile) formData.append("image", imageFile);

      await axiosSecure.put("/about/get-help-victims", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      toast.success("Victims Not Bystanders settings updated successfully!");
      setImageFile(null);
      refetch();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#0F4A63] to-[#156E94] rounded-2xl px-7 py-5 text-white shadow-sm">
        <h1 className="text-xl font-bold tracking-tight">Victims Not Bystanders CMS</h1>
        <p className="text-white/70 text-sm mt-1">
          Manage tagline, bold heading title, paragraph text, and illustration for the Victims Not Bystanders banner.
        </p>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-[#156E94] to-[#0D3B4F] text-white px-8 py-6 relative">
          ...
          <p className="text-white/70 text-xs mt-0.5">
            Modify text paragraphs and illustration graphic shown on the Family & Friends page.
          </p>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              placeholder="Enter paragraph 1 text here..."
              rows={4}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              placeholder="Enter paragraph 2 text here..."
              rows={3}
              className="px-4 py-2.5 rounded-xl border border-slate-200 focus:border-[#156E94] outline-none text-sm transition-all duration-200 text-slate-700 bg-white resize-y"
            />
          </div>

          {/* Right Graphic Upload */}
          <div className="flex flex-col sm:flex-row items-center gap-5 bg-slate-50 p-4 rounded-2xl border border-slate-200/60">
            <div className="w-32 h-32 rounded-xl overflow-hidden bg-white border border-slate-200 flex items-center justify-center relative shadow-sm shrink-0 p-2">
              {imagePreview ? (
                <img
                  src={imagePreview}
                  alt="Preview"
            ...
            <div className="space-y-2 text-center sm:text-left">
              <h4 className="text-sm font-bold text-slate-800">Right Illustration Graphic</h4>
